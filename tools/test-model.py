...
DEFAULT_GRAPH_FILE = os.path.join(SCRIPT_PATH, '../saved-model/optimized_hangeul_tensorflow.pb')
...
def classify(args):
    ...
    # Get relevant nodes.
    x = graph.get_tensor_by_name('hangeul-model/input:0')
    y = graph.get_tensor_by_name('hangeul-model/output:0')
    keep_prob = graph.get_tensor_by_name('hangeul-model/keep_prob:0')
